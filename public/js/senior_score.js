...
document.getElementById('Calc').onclick = function(event) {
    ...
    let total = Number(document.getElementById('govscore').value) +
                Number(document.getElementById('bioscore').value) +
                Number(document.getElementById('mthscore').value) +
                Number(document.getElementById('engscore').value) +
                Number(document.getElementById('fmtscore').value) +
                Number(document.getElementById('ecoscore').value) +
                Number(document.getElementById('litscore').value) +
                Number(document.getElementById('cstscore').value) +
                Number(document.getElementById('civscore').value) +
                Number(document.getElementById('chescore').value) +
                Number(document.getElementById('agrscore').value) +
                Number(document.getElementById('geoscore').value) +
                Number(document.getElementById('physcore').value) +
                Number(document.getElementById('comscore').value) +
                Number(document.getElementById('crsscore').value) +
                Number(document.getElementById('mktscore').value) +
                Number(document.getElementById('accscore').value);

    // Count non-empty scores
    let course = 0;
    const scores = ['govscore', 'bioscore', 'mthscore', 'engscore', 'fmtscore', 'ecoscore', 'litscore', 'cstscore', 'civscore', 'chescore', 'agrscore', 'geoscore', 'physcore', 'comscore', 'crsscore', 'mktscore', 'accscore'];
    scores.forEach(id => {
    ...
    // Promotion logic for SS
    if (term.value === 'THIRD TERM' && sclass.value === 'SS 2' && performance >= 50) {
        report.value += " Promoted to SS 3";
    } else if (term.value === 'THIRD TERM' && sclass.value === 'SS 1' && performance >= 50) {
        report.value += " Promoted to SS 2";
    }
};